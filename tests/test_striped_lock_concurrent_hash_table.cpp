...
#include "striped_lock_concurrent_hash_table.h"
...
void test_concurrent_inserts() {
    ...
    StripedLockConcurrentHashTable<int, int> ht(100);
    int num_threads = 4;
...
void test_concurrent_read_writes() {
    ...
    StripedLockConcurrentHashTable<std::string, int> ht(1000);
...
void test_massive_resize() {
    ...
    StripedLockConcurrentHashTable<int, int> ht(2);
